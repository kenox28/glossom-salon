<?php
/**
 * Glossom Salon — Public landing page with appointment booking.
 */
require_once __DIR__ . '/includes/functions.php';

try {
    $db = getDB();
    $servicesList = $db->query("SELECT id, service_name, price, duration FROM services WHERE is_active = 1 ORDER BY service_name ASC")->fetchAll();
...
<head>
    ...
    <title>Glossom Salon — Premium Salon & Barbershop</title>
...
<body>
    ...
    <section id="appointment">
        <div class="booking-form">
            ...
            <form id="bookingForm" onsubmit="handleFormSubmit(event)" novalidate>
                <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <textarea id="notes" name="notes" placeholder="Any special requests or preferences..."></textarea>
                </div>

                <button type="submit" class="form-submit" id="submitBtn">Book Appointment</button>
            </form>
        </div>
    </section>
    ...
    <script>
        ...
        function handleFormSubmit(event) {
            ...
            .then(res => {
                ...
                msg.style.cssText = `
                    padding: 1.5rem;
                    background: ${res.success ? 'linear-gradient(135deg, #F233C2 0%, #E91F96 100%)' : '#FEF2F2'};
                    color: ${res.success ? 'white' : '#991B1B'};
                    border-radius: 0.6rem;
                    text-align: center;
                    margin-bottom: 1.5rem;
                    animation: slideDown 0.5s cubic-bezier(0.34, 1.56, 0.64, 1);
                    ${res.success ? '' : 'border-left: 3px solid #EF4444;'}
                `;
                msg.innerHTML = `<p style="font-weight:700;font-size:1.1rem;margin:0;">${res.success ? '✓' : '✗'} ${res.message}</p>`;
                form.parentElement.insertBefore(msg, form);

                if (res.success) form.reset();

                setTimeout(() => {
                    msg.style.animation = 'slideUp 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) forwards';
